	...
	IEnumerator Start()
	{
		...
		if (causeDamage)
		{
			foreach (Collider col in cols)
			{
				...
				Health healthScript = col.GetComponent<Collider>().gameObject.GetComponentInParent<Health>();
				if(healthScript != null && !healthScript.gameObject.CompareTag("Player")) healthScript.ChangeHealth(-damageAmount);
