...
public class Weapon : MonoBehaviour
{
	...
	public bool unlimitedMagazine = false;				// Whether or not this weapon should have unlimited magazine
	...
	void Update()
	{
		...
		if (reloadAutomatically && currentAmmo <= 0 && ((reservedAmmo > 0) || unlimitedMagazine) && !isReloading)
	...
	void CheckForUserInput()
	{
		...
		if (type == WeaponType.Raycast)
		{
			if (fireTimer >= actualROF && burstCounter < burstRate && canFire)
			{
				...
				if (warmup && Input.GetButtonUp("Fire1") && Time.timeScale != 0)
				{
		...
		if (Input.GetButtonDown("Reload") && !isReloading && (currentAmmo != ammoCapacity) && ((reservedAmmo > 0) || unlimitedMagazine))
			StartCoroutine(Reload());
	...
	void Fire()
	{
		...
		for (int i = 0; i < shotPerRound; i++)
		{
			...
			if (Physics.Raycast(ray, out hit, range))
			{
				...
				// Damage
				hit.collider.gameObject.SendMessageUpwards("ChangeHealth", -PlayerDamageAttributes.CalculateDamage(damage), SendMessageOptions.DontRequireReceiver);
	...
	void Beam()
	{
		...
		do
		{
			...
			if (Physics.Raycast(ray, out hit, range))
			{
				...
				// Damage
				hit.collider.gameObject.SendMessageUpwards("ChangeHealth", -PlayerDamageAttributes.CalculateDamage(beamPower), SendMessageOptions.DontRequireReceiver);
